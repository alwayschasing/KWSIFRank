...
considered_tags = {'uw','n', 'np', 'ns', 'ni', 'nz','a','d','i','j','x','g'}
class SentEmbeddings():
    ...
    def get_tokenized_sent_embeddings(self, text_obj, if_DS=False, if_EA=False):
        ...
        if(self.embeddings_type=="elmo" and if_DS==False):
            ...
            ed = time.time()
            cost = int((ed - st)*1000)
            #print("get_sent_segmented_cost:%d"%(cost))

            st = time.time()
            elmo_embeddings = self.word_embeddor.get_tokenized_words_embeddings(tokens_segmented)
            ed = time.time()
            cost = int((ed - st)*1000)
            #print("get_tokenized_words_embeddings_cost:%d"%(cost))

            st = time.time()
            elmo_embeddings = context_embeddings_alignment(elmo_embeddings, tokens_segmented)
            ed = time.time()
            cost = int((ed - st)*1000)
            #print("context_embeddings_alignment_cost:%d"%(cost))

            st = time.time()
            elmo_embeddings = splice_embeddings(elmo_embeddings, tokens_segmented)
            ed = time.time()
            cost = int((ed - st)*1000)
            #print("splice_embeddings_cost:%d"%(cost))
        ...
        #check_weight = []
        #for i,tok in enumerate(text_obj.tokens):
        #    check_weight.append([text_obj.tokens[i], weight_list[i]])
        #print("[check_weight]%s\n" % (" ".join(["%s:%f" % (w,s) for [w,s] in check_weight])))
        ed = time.time()
        cost = int((ed - st)*1000)
        #print("get_weight_list_cost:%d"%(cost))
        ...
        ed = time.time()
        cost = int((ed - st)*1000)
        #print("get_weighted_average_cost:%d"%(cost))
        #check_data = {"tokens":text_obj.tokens, "weight_list":weight_list, "sent_embedding":sent_embeddings}
        #checkfp = open("/search/odin/liruihong/keyword-project/SIFRank_zh/check_model_data.bin", "wb")
        ...
        ed = time.time()
        cost = int((ed - st)*1000)
        #print("get_candidate_embeddings_list_cost:%d"%(cost))

        # return sent_embeddings,candidate_embeddings_list,weight_list,elmo_embeddings
...
def splice_embeddings(elmo_embeddings,tokens_segmented):
    ...
    for i in range(1, len(tokens_segmented)):
        ...
        new_elmo_embeddings = numpy.concatenate((new_elmo_embeddings, emb), axis=2)
...
def get_weighted_average(tokenized_sents, sents_tokened_tagged,weight_list, embeddings_list, embeddings_type="elmo"):
    ...
    if (embeddings_type == "elmo" or embeddings_type == "elmo_sectioned"):
        ...
        sum = numpy.zeros((1, embeddings_list.shape[-1]))
        for j in range(0, num_words):
            if (sents_tokened_tagged[j][1] in considered_tags):
                e_test = embeddings_list[j]
                e_test_list.append(e_test)
                sum[0] += e_test * weight_list[j]
        sum[0] = sum[0] / float(num_words)
        return sum
