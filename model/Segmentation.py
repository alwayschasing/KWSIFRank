...
class WordSegmentation(object):
    ...
    def __init__(self, stop_words_file = None, allow_speech_tags = util.allow_speech_tags, use_speech_tag=False):
        """
        Keyword arguments:
        ...
        user_dict="/search/odin/liruihong/keyword-project/SIFRank_zh/auxiliary_data/keyword_vocab_final"
    ...
    def segment(self, text, lower = True, use_stop_words = True, use_speech_tags_filter = False):
        ...
        print_cut = ["%s" % w for w in cut_result]

        if use_speech_tags_filter == True:
            jieba_result = [w for w in cut_result if w[1] in self.default_speech_tag_filter]
        else:
            jieba_result = [w for w in cut_result]

        # 去除特殊符号
        # word_list = [w[0].strip().lower() for w in jieba_result if w[1] !='x']
        word_list = [w[0].strip().lower() for w in jieba_result]
...
class Segmentation(object):
    ...
    def __init__(self, stop_words_file = None,
        ...
        """
        self.ws = WordSegmentation(stop_words_file=stop_words_file, allow_speech_tags=allow_speech_tags, use_speech_tag=use_speech_tag)
